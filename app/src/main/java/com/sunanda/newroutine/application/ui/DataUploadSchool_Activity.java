...
import com.sunanda.newroutine.application.adapter.DataUploadSchool_Adapter;
...
public class DataUploadSchool_Activity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        tvHeaderName.setText("School Upload Data");

        init();
    }

    RecyclerView rvUploadData;
    ArrayList<SampleModel_School> sampleModelsDataCollection;
    DataUploadSchool_Adapter adapter;
    ...
    private void init() {
        ...
        DatabaseHandler databaseHandler = new DatabaseHandler(DataUploadSchool_Activity.this);
        sampleModelsDataCollection = new ArrayList<>();

        String sFCID = CGlobal.getInstance().getPersistentPreference(DataUploadSchool_Activity.this)
                .getString(Constants.PREFS_USER_FACILITATOR_ID, "");

        sampleModelsDataCollection = databaseHandler.getSchoolAppDataCollectionSchool(sFCID, "School");

        adapter = new DataUploadSchool_Adapter(DataUploadSchool_Activity.this, sampleModelsDataCollection, new BtnImage() {
            @Override
            public void onBtnImageValue(final int position) {

                final PrettyDialog prettyDialog = new PrettyDialog(DataUploadSchool_Activity.this);
                                    ...
                                    public void onClick() {
                                        try {
                                            if (!TextUtils.isEmpty(sampleModelsDataCollection.get(position).getSourceImageFile_q_17())) {
                                                ...
                                                try {
                                                    spl = sampleModelsDataCollection.get(position).getSourceImageFile_q_17().split("/");
                                                } catch (Exception e) {
                                                    Log.e("Sample Error", "");
                                                }
                                                try {
                                                    uploadFileSource(spl[spl.length - 1], sampleModelsDataCollection.get(position).getSourceImageFile_q_17(), sampleModelsDataCollection.get(position));
                                                } catch (Exception e) {
                                                    Log.e("Sample Error", "");
                                                }
                                            }
            ...
            public void onBtnImageDeleteValue(final int position) {
                ...
                new AlertDialog.Builder(DataUploadSchool_Activity.this)
                        .setTitle("Delete")
        ...
        RecyclerView.LayoutManager mLayoutManager = new LinearLayoutManager(DataUploadSchool_Activity.this);
    ...
    protected void onResume() {
        super.onResume();
        progressdialog = new ProgressDialog(DataUploadSchool_Activity.this);
                                    ...
                                    public void onClick() {
                                        try {
                                            if (!TextUtils.isEmpty(sampleModelsDataCollection.get(position1).getSourceImageFile_q_17())) {
                                                String spl[] = new String[0];
                                                try {
                                                    spl = sampleModelsDataCollection.get(position1).getSourceImageFile_q_17().split("/");
                                                } catch (Exception e) {
                                                    Log.e("Sample Error", "");
                                                }
                                                try {
                                                    uploadFileSource(spl[spl.length - 1], sampleModelsDataCollection.get(position1).getSourceImageFile_q_17(), sampleModelsDataCollection.get(position1));
                                                } catch (Exception e) {
                                                    Log.e("Sample Error", "");
                                                }
                                            }
    ...
    private void sendData(final SampleModel_School sampleModel) {
        try {
        ...
        try {
            String sUrl = CommonURL.postRutine_1_SchoolApp_URL;
            ...
            CGlobal.getInstance().addVolleyRequest(postRequest, false, DataUploadSchool_Activity.this);
        } catch (Exception e) {
            Log.e("Sample Error", "");
        }
    }

    private void uploadFileSource(String str, String str2, final SampleModel_School sampleModel) {
        ...
        RequestBody type = RequestBody.create(MediaType.parse("text/plain"), "School");
                            ...
                            if (!TextUtils.isEmpty(sampleModel.getImg_sanitary())) {
                                ...
                                try {
                                    if (!TextUtils.isEmpty(sampleModel.getImageofToilet_q_w_2d())) {
                                        String spl[] = new String[0];
                                        try {
                                            spl = sampleModel.getImageofToilet_q_w_2d().split("/");
                                        } catch (Exception e) {
                                            Log.e("Sample Error", "");
                                        }
                                        try {
                                            uploadFileToilet(spl[spl.length - 1], sampleModel.getImageofToilet_q_w_2d(), sampleModel);
                                        } catch (Exception e) {
                                            Log.e("Sample Error", "");
                                        }
                                    } else {
                                        try {
                                            if (!TextUtils.isEmpty(sampleModel.getImageofWashBasin_q_w_3c())) {
                                                String spl[] = new String[0];
                                                try {
                                                    spl = sampleModel.getImageofWashBasin_q_w_3c().split("/");
                                                } catch (Exception e) {
                                                    Log.e("Sample Error", "");
                                                }
                                                try {
                                                    uploadFileWashBasin(spl[spl.length - 1], sampleModel.getImageofWashBasin_q_w_3c(), sampleModel);
                                                } catch (Exception e) {
                                                    Log.e("Sample Error", "");
                                                }
                                            } else {
    ...
    private void uploadFileSurvey(String str, String str2, final SampleModel_School sampleModel) {
    ...
    private void uploadFileToilet(String str, String str2, final SampleModel_School sampleModel) {
    ...
    private void uploadFileWashBasin(String str, String str2, final SampleModel_School sampleModel) {
        try {
            progressdialog.setMessage("Please Wait....");
            progressdialog.show();
        } catch (Exception e) {
